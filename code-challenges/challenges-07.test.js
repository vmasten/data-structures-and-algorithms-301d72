...
const extractAbilities = (arr) => {
  const result = arr.map(snorlax => snorlax.ability.name);
  return result;
};
...
const extractStats = (arr) => {
  const result = arr.map(snorlax => {
    return {'name': snorlax.stat.name, 'total': snorlax.baseStat + snorlax.effort};
  });
  return result;
};
