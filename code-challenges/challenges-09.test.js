...
function transformToLis(obj){
  ...
  for (const key in obj) {
    results.push(`<li>${key}: ${obj[key]}</li>`);
  }
  return results;
...
const addPurchases = (arr) => {
  return arr.reduce((added, purchase) => added += purchase.purchasePrice, 0);
};
...
const countNumberOfElements = (arr) => {
  return arr.reduce((added, ele) => added += 1, 0);
  //This looks pretty ugly, but it works
};
...
const returnNames = (arr) => {
  const newArray = arr.reduce((added, person) => {
...
const reversedString = (str) => {
  const strArray = [];
  for (let char in str) {
    strArray.unshift(str[char]);
  }
  const result = strArray.reduce((reversed, char) => {
    reversed += char;
    return reversed;
  },'');
  return result;
