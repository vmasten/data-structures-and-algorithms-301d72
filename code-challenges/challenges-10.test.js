...
const getNames = (arr) => {
  return arr.map(item => {
    const arrSplit = item.name.split('');
    const isReversed = arrSplit.reverse();
    let finalName = '';
    for (const letter of isReversed) {
      finalName += letter;
    }
    return finalName;
  });
...
------------------------------------------------------------------------------------------------ */
//once I figured out #2 using reduce, I just had to adapt it lightly
...
const divisibleByFiveTwoToThePower = (input) => {
  ...
  for (const arr of input) {
    let mappedArr = [];
    testResult = arr.map(item => {
      if (item % 5 === 0) {
        mappedArr.push(Math.pow(2, item));
      }
    });
